...
public class TodoDao {
    ...
    public TodoDto getData(int num){
        //select 할 때는 getReadableDatabase() 메소드를 호출한다.
        ...
        if(result.moveToNext()){
            //커서가 위치한 곳의 row 에서 0번째 칼럼에 있는 문자열 얻어오기
            String content=result.getString(0);
            //커서가 위치한 곳의 row 에서 1번째 칼럼에 있는 문자열 얻어오기
    ...
    public List<TodoDto> getList(){
        ...
        */
        String sql="SELECT num,content," +
                ...
                " FROM todo" +
                " ORDER BY num ASC";        Cursor result=db.rawQuery(sql,null);
